...
public class NoSatisfactionWithTypeTest {
	...
	public void BudgetEnoughForSomeItemWithCompulsoryItem() throws Exception {
		...
		boolean compareResult = true;
		if(cb.getSolutionSize() != 3) //inclusive of empty set 
			compareResult = false;
		if(cb.getSolutionCost(0) != 0.0) //empty set
			compareResult = false;
		if(cb.getSolutionCost(1) != 64.7)
			compareResult = false;
		if(cb.getSolutionCost(2) != 49.99)
			compareResult =false;
